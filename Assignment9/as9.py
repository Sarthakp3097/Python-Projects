...
class Complex_Number:
    ...
    def __pow__(self,exponent):
        final = Complex_Number(self.rpart, self.ipart)
...
# os.chdir("D:\C200\Fall 2023\Assignments\Assignment 9")
...
def query2(db_cursor):
    result = []
    for i in db_cursor.execute("SELECT * FROM Weather WHERE High < 80"):
        result.append(i)
    return result
...
def query4(db_cursor):
    result = []
    for i in db_cursor.execute("Select City, Low FROM Weather ORDER BY Low LIMIT 1"):
        result.append(i)
    return result

def query5(db_cursor):
    result = []
    for i in db_cursor.execute("Select City, High FROM Weather ORDER BY High DESC LIMIT 2"):
        result.append(i)
    return result
def query6(db_cursor):
    result = []
    for i in db_cursor.execute("Select AVG(HIGH), AVG(Low) FROM Weather"):
        result.append(i)
    return result

def query7(db_cursor):
    result = []
    for i in db_cursor.execute("Select Low, COUNT(*) FROM Weather GROUP BY Low"):
        result.append(i)
    return result
#TEST CASES
